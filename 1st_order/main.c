...
int main() {
	...
	char output_filename[50] = "result_spambase_1.csv";
	char table_output_filename[50] = "table_spambase.csv";

	double lambda1 = 0;
	double lambda2 = pow(10, -6);
	...
	for (i = no_exp; i > 0; i--) {
		...
		obj_value = __optimizer(w, X_matrix, Y_array, lambda1, 1.0*i*lambda2, data_dim, data_num, 30, 4*data_num, max_iter, epsilon);
		end = clock();

		time_lapse = 1.0*(end - start) / CLOCKS_PER_SEC;
		printf("FISTA (lambda1 = E%3.2f, lambda2 = E%3.2f) obj = %3.10lf in %3.3lf sec\n", log(lambda1) / log(10), log(lambda2) / log(10), obj_value, time_lapse);
		printf("--------------------------------------\n");
